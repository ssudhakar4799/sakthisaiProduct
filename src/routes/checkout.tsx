...
export const Route = createFileRoute("/checkout")({
  ...
  head: () => ({
    meta: [
      { title: "Checkout — Silprovet" },
      { name: "description", content: "Complete your Silprovet order. Fast shipping and secure checkout." },
    ],
  }),
});
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto flex flex-col items-center justify-center px-4 py-20 text-center">
          ...
          <p className="animate-fade-up mt-3 max-w-md text-muted-foreground" style={{ animationDelay: "0.1s" }}>
            Thank you! Your order <span className="font-semibold text-primary">#{orderId}</span> has been received. A confirmation email with shipping details is on its way.
          </p>
          ...
          <div className="animate-fade-up mt-8 flex flex-col gap-3 sm:flex-row" style={{ animationDelay: "0.3s" }}>
            <Button onClick={() => setConfirmed(false)} variant="hero" size="lg" className="rounded-full px-8">
              Place Another Order
            </Button>
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    className={`h-10 w-10 rounded-lg border font-medium transition-colors ${quantity === n ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:border-primary"}`}
                  >
                    {n}
                  </button>
                ))}
            ...
            <div className="rounded-2xl bg-card p-6 shadow-sm">
              ...
              <RadioGroup value={shipping} onValueChange={setShipping} className="space-y-2">
                <Label className="flex cursor-pointer items-center justify-between rounded-lg border border-border p-3 hover:border-primary">
                  <span className="flex items-center gap-2"><RadioGroupItem value="standard" /> Standard (5-7 days)</span>
                  <span className="font-medium">$5.00</span>
                </Label>
                <Label className="flex cursor-pointer items-center justify-between rounded-lg border border-border p-3 hover:border-primary">
                  <span className="flex items-center gap-2"><RadioGroupItem value="express" /> Express (1-2 days)</span>
                  <span className="font-medium">$12.00</span>
                </Label>
                <Label className="flex cursor-pointer items-center justify-between rounded-lg border border-border p-3 hover:border-primary">
                  <span className="flex items-center gap-2"><RadioGroupItem value="free" /> Free pickup</span>
